...
num_element_bytes = case out_format
  when :mono then 1
  when :rgb565 then 2
  when :rgbx32 then 4
end
...
elements_per_line = 120 / num_out_element_digits

out_source = "// Do not edit; generated by maketcwimage.rb\n"
out_source << "\#include \"#{out_name}.h\"\n\n"
out_source << "const #{out_type_name} #{symbol_name}[] = {\n"
...
num_out_lines.times do |line|
  ...
  out_elements[line * elements_per_line, elements_per_line].each do |element|
    element_str = ("000000" + element.to_s(16))[-num_out_element_digits, num_out_element_digits]
...
out_source << "const TWBitmapInfoHeader #{symbol_name}_InfoHeader = {\n"
out_source << "  #{symbol_name}_Width, #{symbol_name}_Height,\n"
out_source << "  #{symbol_name}_PixelFormat,\n"
out_source << "};\n"
...
out_header << "extern const #{out_type_name} #{symbol_name}[#{out_elements.length}];\n"
out_header << "extern const TWBitmapInfoHeader #{symbol_name}_InfoHeader;\n"
out_header << "\#define #{symbol_name}_Data ((const char *)#{symbol_name})\n"
out_header << "\#define #{symbol_name}_Size #{out_elements.length * num_element_bytes}\n"
