...
static inline uint16_t TWColorToRGB565(TWColor color) {
	uint16_t ret = color.red >> 3;
	ret |= ((uint16_t)(color.green >> 2)) << 5;
	ret |= ((uint16_t)(color.blue >> 3)) << 11;
	return ret;
}

static inline uint16_t RGBX32ToRGB565(uint32_t rgbx32) {
	uint16_t ret = (rgbx32 >> 3) & 0x1f;
	ret |= ((uint16_t)(rgbx32 >> 5)) & 0x7e0;
	ret |= ((uint16_t)(rgbx32 >> 8)) & 0xf800;
	return ret;
}
 ...
 * global_name:  tTWRGB565BitmapGraphicsRenderer_eGraphicsDeviceOutput_setClippingRect
 * oneway:       true
 * #[</ENTRY_FUNC>]# */
void
 ...
 * global_name:  tTWRGB565BitmapGraphicsRenderer_eGraphicsDeviceOutput_subtractClippingRect
 * oneway:       true
 * #[</ENTRY_FUNC>]# */
void
...
eGraphicsDeviceOutput_fillRect(CELLIDX idx, TWColor color, const TWRect* rect)
{
	...
	uint_fast16_t pixel = TWColorToRGB565(color);
...
eGraphicsDeviceOutput_drawBitmap(CELLIDX idx, const char* data, uint32_t numBytes, const TWBitmapInfo* info, uint16_t infoSize, const TWRect* inRect, const TWPoint* outLoc)
{
	...
	switch (format) {
		case TWPixelFormat1bppMonotone: {
			uint_fast16_t pixel1 = TWColorToRGB565(info->mono.palette[0]);
			uint_fast16_t pixel2 = TWColorToRGB565(info->mono.palette[1]);
			data += in_x1 >> 3;
		...
		case TWPixelFormat32bppRGBX: {
			...
			do {
				...
				do {
					...
					for (int i = 0; i < spanScanner.width; ++i) {
						*(pixels2++) = RGBX32ToRGB565(*(in_pixels2++));
					}
				} while (TWScanlineClipperLineAdvance(&spanScanner));
